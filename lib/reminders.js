...
export async function runDailyReminderMaintenance() {
  ...
  try {
    ...
    // ② 有効期限チェック:投入待ち/表示待ちのまま90日経過したものを除外
    ...
    const { error: expireError } = await supabase
      ...
      .lt("created_at", expireBefore.toISOString());
...
export async function getDueReminderCards() {
  try {
    ...
    const { data, error } = await supabase
      .from("reminder_queue")
      .select(
        "id, card_id, interval_stage, next_show_at, brain_logs!inner(id, title, content, tags, created_at, deleted_at)"
      )
      .eq("status", "queued")
