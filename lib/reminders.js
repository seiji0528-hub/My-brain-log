...
export async function getDueReminderCards() {
  try {
    ...
    const { data, error } = await supabase
      .from("reminder_queue")
      .select(
        "id, card_id, interval_stage, next_show_at, brain_logs!inner(id, title, content, tags, created_at, deleted_at, reference_type, reference_value)"
      )
      .eq("status", "queued")
    ...
    return (data || [])
      ...
      .map((row) => ({
        ...
        referenceType: row.brain_logs.reference_type || null,
        referenceValue: row.brain_logs.reference_value || null,
      }));
  } catch (e) {
