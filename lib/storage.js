...
export async function loadCards() {
  try {
    ...
    return (data || []).map((item) => ({
      ...
      references: Array.isArray(item.attachments) ? item.attachments : [],
    }));
  } catch (e) {
...
export async function updateCard(id, { title, body, tags, references }) {
  try {
    const payload = {
      title,
      content: body,
    ...
    // references が渡された時だけ更新する(未指定なら参考資料は変更しない)
...
// --- 以下、既存の便利機能 ---
export function createCard({ title, body, tags, references }) {
  return {
