      ...
      .insert([
        {
          ...
          content: card.body,
          tags: card.tags || [],
          reference_type: card.referenceType || null,
          reference_value: card.referenceValue || null,
        },
      ])
      .select();
...
export async function updateCard(id, { title, body, tags, referenceType, referenceValue }) {
  try {
    const payload = {
      title,
      content: body,
    ...
    // referenceType が渡された時だけ更新する(未指定なら参考資料は変更しない)
    if (referenceType !== undefined) {
      payload.reference_type = referenceType || null;
      payload.reference_value = referenceValue || null;
    }
...
// --- 以下、既存の便利機能 ---
export function createCard({ title, body, tags, referenceType, referenceValue }) {
  return {
...
const REFERENCE_BUCKET = 'card-references';

// --- 参考資料の画像をアップロードし、公開URLを返す ---
export async function uploadReferenceImage(file) {
  try {
    ...
    if (uploadError) {
      console.error('参考資料の画像アップロードエラー:', uploadError);
      throw uploadError;
    }
    ...
    console.error('参考資料の画像アップロードに失敗しました', e);
    throw e;
  }
}

// --- 参考資料の画像を削除する(差し替え・削除時の後始末。失敗しても致命的ではないので投げない) ---
export async function deleteReferenceImage(publicUrl) {
  try {
    if (!publicUrl) return;
    const marker = `/${REFERENCE_BUCKET}/`;
    const idx = publicUrl.indexOf(marker);
    if (idx === -1) return;
    const path = publicUrl.slice(idx + marker.length);
    const { error } = await supabase.storage.from(REFERENCE_BUCKET).remove([path]);
    if (error) {
      console.error('参考資料の画像削除エラー:', error);
    }
  } catch (e) {
    console.error('参考資料の画像削除に失敗しました', e);
  }
}
